...
public class SetWaypointsCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new UnitArgument(UNIT_ID, Messages.getString("Caspar.command.setWaypoints.unitID")),
              new MultiHexNumberArgument(HEX_NUMBER, Messages.getString("Caspar.command.setWaypoints.waypoints")),
              quietArgument()
        );
    }

    @Override
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        Optional<Entity> unitOpt = Caspar.getEntitiesOwned().stream()
              .filter(entity -> entity.getId() == unitArgument.getValue())
              .findFirst();

        if (unitOpt.isEmpty()) {
            if (!quietArgument.getValue()) {
                Caspar.sendChat(Messages.getString("Caspar.command.setWaypoints.unitNotFound",
        ...
        for (var coords : multiHexNumberArgument.getValue()) {
            if (!Caspar.getGame().getBoard().contains(coords) && !quietArgument.getValue()) {
                Caspar.sendChat(Messages.getString("Caspar.command.setWaypoints.boardNoHex",
                      coords.toFriendlyString()));
                return;
            }
        }

        Caspar.getUnitBehaviorTracker()
              .setEntityWaypoints(unitOpt.get(), multiHexNumberArgument.getValue(), Caspar);

        if (!quietArgument.getValue()) {
            Caspar.sendChat(Messages.getString("Caspar.command.setWaypoints.success",
