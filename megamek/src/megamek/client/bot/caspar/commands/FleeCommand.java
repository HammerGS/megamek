...
public class FleeCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new EnumArgument<>(EDGE, Messages.getString("Caspar.command.flee.edge"), CardinalEdge.class)
        );
    }
    ...
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        Caspar.sendChat("Received flee order - " + edge.name());
        Caspar.getCasparSettings().setDestinationEdge(edge);
        Caspar.setFallBack(true, "Received flee order - " + edge.name());
    }
}
