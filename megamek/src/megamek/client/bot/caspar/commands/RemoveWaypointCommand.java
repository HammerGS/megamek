...
public class RemoveWaypointCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new UnitArgument(UNIT_ID, Messages.getString("Caspar.command.removeWaypoint.unitID")),
              quietArgument()
        );
    }
    ...
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        if (unitOpt.isEmpty()) {
            if (!quietArgument.getValue()) {
                Caspar.sendChat(Messages.getString("Caspar.command.removeWaypoint.unitNotFound",
        ...
        Caspar.getUnitBehaviorTracker().removeTailWaypoint(unitOpt.get());
        if (!quietArgument.getValue()) {
            Caspar.sendChat(Messages.getString("Caspar.command.removeWaypoint.success",
