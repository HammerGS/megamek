...
public class CautionCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new IncDecSetIntegerArgument(
                    CAUTION,
                    Messages.getString("Caspar.command.caution.caution"),
                    0,
                    10)
        );
    }
    ...
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        int currentIndex = Caspar.getCasparSettings().getFallShameIndex();

        if (value.getOperation().equals(IncDecSetIntegerArgument.Operation.SET)) {
            Caspar.getCasparSettings().setFallShameIndex(value.getValue());
        } else {
            Caspar.getCasparSettings().setFallShameIndex(
                  Caspar.getCasparSettings().getFallShameIndex() + value.getValue());
        }

        Caspar.sendChat(Messages.getString("Caspar.command.caution.cautionChanged",
              currentIndex, Caspar.getCasparSettings().getFallShameIndex()));
    }
}
