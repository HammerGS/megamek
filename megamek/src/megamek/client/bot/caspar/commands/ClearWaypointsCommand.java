...
public class ClearWaypointsCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new UnitArgument(UNIT_ID, Messages.getString("Caspar.command.clearWaypoints.unitId")),
              quietArgument()
        );
    }
    ...
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        if (unitOpt.isEmpty()) {
            if (!quietArgument.getValue()) {
                Caspar.sendChat(Messages.getString("Caspar.command.clearWaypoints.unitNotFound",
                      unitArgument.getValue()));
            }
            return;
        }

        Caspar.getUnitBehaviorTracker().clearWaypoints(unitOpt.get());
        if (!quietArgument.getValue()) {
            Caspar.sendChat(Messages.getString("Caspar.command.clearWaypoints.success",
