...
public class TargetGroundCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new HexNumberArgument(HEX, Messages.getString("Caspar.command.targetGround.hex")),
              quietArgument()
        );
    }
    ...
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        if (!Caspar.getGame().getBoard().contains(hexArg.getValue())) {
            if (!quietArg.getValue()) {
                Caspar.sendChat(Messages.getString("Caspar.command.targetGround.hexNotFound",
                      hexArg.getValue().toFriendlyString()));
            }
            return;
        }

        Caspar.addStrategicBuildingTarget(hexArg.getValue());
        if (!quietArg.getValue()) {
            Caspar.sendChat(Messages.getString("Caspar.command.targetGround.success",
                  hexArg.getValue().toFriendlyString()));
        }
    }
}
