...
public class HerdingCommand implements ChatCommand {
    ...
    public List<Argument<?>> defineArguments() {
        return List.of(
              new IncDecSetIntegerArgument(
                    HERDING,
                    Messages.getString("Caspar.command.herding.herding"),
                    0,
                    10)
        );
    }
    ...
    public void execute(Caspar Caspar, Arguments arguments) {
        ...
        int currentIndex = Caspar.getCasparSettings().getHerdMentalityIndex();

        if (value.getOperation().equals(IncDecSetIntegerArgument.Operation.SET)) {
            Caspar.getCasparSettings().setHerdMentalityIndex(value.getValue());
        } else {
            Caspar.getCasparSettings().setHerdMentalityIndex(
                  Caspar.getCasparSettings().getHerdMentalityIndex() + value.getValue());
        }
        Caspar.sendChat(Messages.getString("Caspar.command.herding.herdingChanged",
              currentIndex, Caspar.getCasparSettings().getHerdMentalityIndex()));
    }
}
