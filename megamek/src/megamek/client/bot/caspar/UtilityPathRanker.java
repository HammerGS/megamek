...
public class UtilityPathRanker extends BasicPathRanker {
    ...
    public UtilityPathRanker(Caspar owningCaspar) {
        super(owningCaspar);
    }
    ...
    protected double getBraveryMod(double successProbability, FiringPhysicalDamage damageEstimate,
        ...
        double braveryFactor = getOwner().getCasparSettings().getBraveryIndex() / 10.0;
    ...
    protected double calculateMovementMod(MovePath pathCopy, Game game) {
        var tmmFactor = getOwner().getCasparSettings().getFavorHigherTMM() / 10.0;
    ...
    protected double calculateFallMod(double successProbability) {
        double fallShameFactor = getOwner().getCasparSettings().getFallShameIndex() / 10.0;
    ...
    protected double calculateAggressionMod(Entity movingUnit, MovePath path, double maxRange, Game game) {
        ...
        double weight = getOwner().getCasparSettings().getHyperAggressionIndex() / 10.0;
    ...
    protected double calculateSelfPreservationMod(Entity movingUnit, MovePath path, Game game) {
        ...
        double weight = getOwner().getCasparSettings().getSelfPreservationIndex() / 10.0;
