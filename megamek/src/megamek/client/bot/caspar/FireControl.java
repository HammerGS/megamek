...
public class FireControl {
    ...
    protected final Caspar owner;

    /**
     * Constructor
     *
     * @param owningCaspar The {@link Caspar} bot that utilizes this class for computing firing solutions.
     */
    public FireControl(final Caspar owningCaspar) {
        owner = owningCaspar;
    }
     ...
     * @param shooterState          The {@link EntityState} of the unit doing the shooting.
     * @param target                The {@link Targetable} being shot at.
     * @param targetState           The {@link megamek.client.bot.caspar.EntityState} of the unit being shot at.
    ...
    protected double calcStrategicBuildingTargetUtility(final Targetable target, final double expectedDamage) {
        ...
        if (owner.getCasparSettings().getStrategicBuildingTargets().contains(coords)) {
     ...
     * often). In practice, this utility term results in Caspar concentrating her fire enough to reliably kill/cripple
     ...
     * potential damage, the weight value TARGET_POTENTIAL_DAMAGE_UTILITY, and Caspar's self-preservation value. This
    ...
    protected double calcTargetPotentialDamageMultiplier(final Targetable target) {
        ...
        final double self_preservation = owner.getCasparSettings().getSelfPreservationValue();
        final double max_self_preservation = owner.getCasparSettings()
        ...
        for (final WeaponFireInfo weaponFireInfo : alphaStrike) {
            ...
            if (shooter instanceof Infantry) {
                if ((weaponFireInfo.getWeapon().getType()).getInternalName().equals(Infantry.LEG_ATTACK)) {
                    ...
                    // We probably shouldn't consider stopping swarm attacks, since Caspar isn't
    ...
    FiringPlan getBestFiringPlan(final Entity shooter,
        ...
        for (final Targetable enemy : enemies) {
            if (owner.getCasparSettings().getIgnoredUnitTargets().contains(enemy.getId())) {
