 ...
 */
public class CasparSettingsFactory {
    private final static MMLogger logger = MMLogger.create(CasparSettingsFactory.class);

    private static final String CASPAR_BEHAVIOR_PATH = "mmconf" + File.separator + "casparBehaviors.xml";

    final Map<String, CasparSettings> behaviorMap = new HashMap<>();
    private static final CasparSettingsFactory instance = new CasparSettingsFactory();

    private CasparSettingsFactory() {
        init(true);
    }

    public static CasparSettingsFactory getInstance() {
        return instance;
    }

    /**
     * Initializes the {@link megamek.client.bot.caspar.CasparSettings} cache. If the cache is empty, it will load
     * from mmconf/CasparBehaviors.xml. Also, if the "DEFAULT" behavior is missing, it will be added.
     *
    ...
    public void init(boolean reinitialize) {
        synchronized (behaviorMap) {
            ...
            if (behaviorMap.isEmpty()) {
                loadCasparSettings(buildCasparBehaviorDoc());
            }
            addDefaultBehaviors();
        }
    }
    ...
    /**
     * Adds a {@link megamek.client.bot.caspar.CasparSettings} to the cache. If a behavior with the same name is
     * already in the cache, it will be overwritten.
     *
     * @param CasparSettings The {@link megamek.client.bot.caspar.CasparSettings} to be added to the cache.
     */
    public void addBehavior(CasparSettings CasparSettings) {
        synchronized (behaviorMap) {
            behaviorMap.put(CasparSettings.getDescription().trim(), CasparSettings);
        }
    }

    /**
     * Removes the behavior setting with the given name from the cache. Returns the CasparSettings that was removed
     * (or null if there was no such CasparSettings).
     */
    ...
    /**
     * Returns the named {@link megamek.client.bot.caspar.CasparSettings}.
     *
     * @param desc The name of the behavior; matched to
     *             {@link megamek.client.bot.caspar.CasparSettings#getDescription()}.
     *
     * @return The named behavior or NULL if no match is found.
     */
    public CasparSettings getBehavior(String desc) {
     ...
     */
    public CasparSettings getBehaviorOrDefault(String desc, CasparSettings defaultBehavior) {
        return behaviorMap.getOrDefault(desc, defaultBehavior);
    }

    private @Nullable Document buildCasparBehaviorDoc() {
        try {
            File behaviorFile = new File(CASPAR_BEHAVIOR_PATH);
            if (!behaviorFile.exists() || !behaviorFile.isFile()) {
                String message = String.format("Could not load %s", CASPAR_BEHAVIOR_PATH);
            ...
            logger.error(ex, "Build Caspar Exception");
            return null;
        }
    }

    /**
     * Loads the contents of the mmconf/CasparBehaviors.xml file into the cache. If the "DEFAULT" behavior is missing
     * it will be automatically added.
     *
     * @return TRUE if the load completes successfully.
     */
    boolean loadCasparSettings(Document CasparBehaviorDoc) {
        synchronized (behaviorMap) {
            addDefaultBehaviors();

            if (CasparBehaviorDoc == null) {
                return false;
            }

            Element root = CasparBehaviorDoc.getDocumentElement();
            CasparSettings CasparSettings;
            ...
            try {
                for (int i = 0; i < root.getChildNodes().getLength(); i++) {
                    ...
                    CasparSettings = new CasparSettings((Element) child);
                    addBehavior(CasparSettings);
                }
            } catch (Exception e) {
    ...
    /**
     * Saves the contents of the cache to the mmconf/CasparBehaviors.xml file.
     *
     * @param includeTargets Set TRUE to include the contents of the Strategic Targets list.
     */
    public void saveCasparSettings(boolean includeTargets) {
        init(false);
        ...
        try {
            ...
            if (!behaviorFile.exists()) {
                if (!behaviorFile.createNewFile()) {
                    String message = String.format("Could not create %s", CASPAR_BEHAVIOR_PATH);
                    logger.error(message);
                    return;
                }
            }
            ...
            if (!behaviorFile.canWrite()) {
                String message = String.format("Could not write to %s", CASPAR_BEHAVIOR_PATH);
                logger.error(message);
                return;
            }

            Document behaviorDoc = MMXMLUtility.newSafeDocumentBuilder().newDocument();
            Node rootNode = behaviorDoc.createElement("CasparBehaviors");

            synchronized (behaviorMap) {
                for (String key : behaviorMap.keySet()) {
                    CasparSettings settings = behaviorMap.get(key);
    ...
    /**
     * @return an array of the names of all the {@link megamek.client.bot.caspar.CasparSettings} in the cache.
     */
     ...
     * {@link megamek.client.bot.caspar.CasparSettings CasparSettings}.
     */
     ...
     */
    // Used by MekHQ
    public final CasparSettings BERSERK_BEHAVIOR = buildBerserkBehavior();
    public static final String BERSERK_BEHAVIOR_DESCRIPTION = "BERSERK";

    private CasparSettings buildBerserkBehavior() {
        try {
            CasparSettings berserkBehavior = new CasparSettings();
            ...
            berserkBehavior.setNumberOfEnemiesToConsiderFacing(CasparSettings.DEFAULT_NUMBER_OF_ENEMIES_TO_CONSIDER_FACING);
            berserkBehavior.setAllowFacingTolerance(CasparSettings.DEFAULT_ALLOW_FACING_TOLERANCE);
     ...
     * Strategic Targets: None
     */
    public final CasparSettings COWARDLY_BEHAVIOR = buildCowardlyBehavior();
    public static final String COWARDLY_BEHAVIOR_DESCRIPTION = "COWARDLY";

    private CasparSettings buildCowardlyBehavior() {
        try {
            CasparSettings cowardlyBehavior = new CasparSettings();
            ...
            cowardlyBehavior.setNumberOfEnemiesToConsiderFacing(CasparSettings.DEFAULT_NUMBER_OF_ENEMIES_TO_CONSIDER_FACING);
            cowardlyBehavior.setAllowFacingTolerance(CasparSettings.DEFAULT_ALLOW_FACING_TOLERANCE);
     ...
     */
    // Used by MekHQ
    public final CasparSettings ESCAPE_BEHAVIOR = buildEscapeBehavior();
    public static final String ESCAPE_BEHAVIOR_DESCRIPTION = "ESCAPE";

    private CasparSettings buildEscapeBehavior() {
        try {
            CasparSettings escapeBehavior = new CasparSettings();
            ...
            escapeBehavior.setNumberOfEnemiesToConsiderFacing(CasparSettings.DEFAULT_NUMBER_OF_ENEMIES_TO_CONSIDER_FACING);
            escapeBehavior.setAllowFacingTolerance(CasparSettings.DEFAULT_ALLOW_FACING_TOLERANCE);
     ...
     */
    // Used by MekHQ
    public final CasparSettings RUTHLESS_BEHAVIOR = buildRuthlessBehavior();
    public static final String RUTHLESS_BEHAVIOR_DESCRIPTION = "RUTHLESS";

    private CasparSettings buildRuthlessBehavior() {
        try {
            CasparSettings ruthlessBehavior = new CasparSettings();
     ...
     */
    // Used by MekHQ
    public final CasparSettings PIRATE_BEHAVIOR = buildPirateBehavior();
    public static final String PIRATE_BEHAVIOR_DESCRIPTION = "PIRATE";

    private CasparSettings buildPirateBehavior() {
        try {
            CasparSettings pirateBehavior = new CasparSettings();
     ...
     */
    // Used by MekHQ
    public final CasparSettings CONVOY_BEHAVIOR = buildConvoyBehavior();
    public static final String CONVOY_BEHAVIOR_DESCRIPTION = "CONVOY";

    private CasparSettings buildConvoyBehavior() {
        try {
            CasparSettings convoyBehavior = new CasparSettings();
     ...
     */
    public final CasparSettings DEFAULT_BEHAVIOR = buildDefaultBehavior();
    public static final String DEFAULT_BEHAVIOR_DESCRIPTION = "DEFAULT";

    private CasparSettings buildDefaultBehavior() {
        try {
            CasparSettings defaultBehavior = new CasparSettings();
            ...
            defaultBehavior.setNumberOfEnemiesToConsiderFacing(CasparSettings.DEFAULT_NUMBER_OF_ENEMIES_TO_CONSIDER_FACING);
            defaultBehavior.setAllowFacingTolerance(CasparSettings.DEFAULT_ALLOW_FACING_TOLERANCE);
            defaultBehavior.setExclusiveHerding(false);
            return defaultBehavior;
        } catch (Exception e) {
            logger.error(e, "Default Behavior Exception");
            return new CasparSettings();
        }
    }
}
